...
const chartsReducer = (state: ChartsState, action: Action) => {
  switch (action.type) {
    case ActionsType.ADD_CHART:
      if (!action.newChart)
        throw Error('newChart must be defined to ChartsContext.ADD_CHART');
      return {
        ...state,
    ...
    case ActionsType.REMOVE_CHART:
      if (action.chartId === undefined)
        throw Error('chartId must be defined to ChartsContext.REMOVE_CHART');
      return {
        ...state,
          ...
          chart.id === action.chartId
            ? {
                ...
                exercises: chart.exercises.map(exercise =>
                  exercise.id == action.exerciseId ? action.exercise : exercise,
                ),
              }
            : chart,
        ) as Chart[],
    ...
    case ActionsType.SELECT_CHART:
      if (action.chartId === undefined)
        throw Error('chartId must be defined to ChartsContext.SELECT_CHART');
...
export const useCharts = () => {
  ...
  const addExercise = (chartId: number) => {
    ...
    const newExerciseName = getUniqueName('Novo exercício', chart.exercises);
